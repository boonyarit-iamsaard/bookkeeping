...
import type { WalletSummary } from "@/features/wallets/server/operations";
...
export function WalletList({ wallets, createdId }: Readonly<WalletListProps>) {
  ...
  const total = wallets.reduce((sum, wallet) => sum + wallet.balance, 0n);
  const countLabel =
    ...
    <div className="flex flex-col gap-8">
      <section aria-labelledby="total-balance-heading">
        ...
        <p className="text-4xl leading-none sm:text-5xl">
          <Money amountInMinorUnits={total} currency="THB" display />
        </p>
